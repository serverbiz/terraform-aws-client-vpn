...
| <a name="input_server_dns_names"></a> [server\_dns\_names](#input\_server\_dns\_names) | DNS Subject Alternative Names (SANs) for the server certificate. When empty, falls back to the server_subject common_name if one is set. Provide a SAN here (or a common_name) to avoid certificates that rely only on the legacy Common Name field, which Go 1.15+ TLS stacks used by many OpenVPN clients reject. | `list(string)` | `[]` | no |
